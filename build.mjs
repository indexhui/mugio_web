...
function renderLocale(locale) {
  ...
  const canonicalUrl = `${siteOrigin}${config.route}`;
  let html = sourceHtml;
  ...
  html = html
    ...
    .replace(/<title>[\s\S]*?<\/title>/, `<title>${escapeHtml(dictionary["meta.title"])}</title>`)
    .replace(/(<meta name="description" content=")[^"]*(" \/>)/, `$1${escapeHtml(dictionary["meta.description"])}$2`)
    .replace(/(<meta property="og:title" content=")[^"]*(" \/>)/, `$1${escapeHtml(dictionary["meta.title"])}$2`)
    .replace(/(<meta property="og:description" content=")[^"]*(" \/>)/, `$1${escapeHtml(dictionary["meta.description"])}$2`)
    ...
    .replace('rel="alternate" hreflang="zh-Hant" href="/zh-TW"', `rel="alternate" hreflang="zh-Hant" href="${siteOrigin}/zh-TW"`)
    .replace('rel="alternate" hreflang="en" href="/en"', `rel="alternate" hreflang="en" href="${siteOrigin}/en"`)
    .replace('rel="alternate" hreflang="ja" href="/ja"', `rel="alternate" hreflang="ja" href="${siteOrigin}/ja"`)
    .replace('rel="alternate" hreflang="x-default" href="/zh-TW"', `rel="alternate" hreflang="x-default" href="${siteOrigin}/zh-TW"`)
    .replace('href="/zh-TW" lang="zh-Hant" hreflang="zh-Hant" data-lang="zh-TW" aria-current="page"', `href="/zh-TW" lang="zh-Hant" hreflang="zh-Hant" data-lang="zh-TW"${locale === "zh-TW" ? ' aria-current="page"' : ""}`)
    .replace('href="/en" lang="en" hreflang="en" data-lang="en"', `href="/en" lang="en" hreflang="en" data-lang="en"${locale === "en" ? ' aria-current="page"' : ""}`)
    .replace('href="/ja" lang="ja" hreflang="ja" data-lang="ja"', `href="/ja" lang="ja" hreflang="ja" data-lang="ja"${locale === "ja" ? ' aria-current="page"' : ""}`)
...
for (const locale of Object.keys(locales)) {
  ...
  await writeFile(path.join(localeDir, "index.html"), renderLocale(locale), "utf8");
}

await writeFile(path.join(outputRoot, "index.html"), renderLocale("zh-TW"), "utf8");
...
console.log("Built static language routes: /zh-TW, /en, /ja");
